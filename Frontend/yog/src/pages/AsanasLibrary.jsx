...
const AsanasLibrary = () => {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar remains fixed and unaffected by loading */}
      <div className="bg-gray-900 text-white">
        <Sidebar />
      </div>
      ...
      <div className="flex-1 bg-gradient-to-br from-gray-100 to-gray-300 dark:from-gray-900 dark:to-gray-800 p-6">
        <div className="container mx-auto">
          ...
          {/* ✅ Loading state only affects the right-side content */}
          {loading ? (
            <div className="flex justify-center items-center h-96 text-white">
              <p className="text-xl">Loading asanas...</p>
            </div>
          ) : error ? (
            <div className="p-4 bg-red-600 text-white rounded mt-4">{error}</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredAsanas.length > 0 ? (
                filteredAsanas.map((asana) => (
